...
namespace rb {
    ...
    private:
        std::vector<VkDescriptorPoolSize> _create_pool_sizes(const resource_heap_desc& desc);

        void _create_descriptor_pool(const resource_heap_desc& desc);

        void _create_descriptor_sets(const resource_heap_desc& desc);

        void _update_descriptor_sets(const resource_heap_desc& desc);

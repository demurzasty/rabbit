...
material_vulkan::material_vulkan(VkDevice device, VmaAllocator allocator, const material_desc& desc)
    : material(desc)
    , _device(device)
    ...
    void* data;
    result = vmaMapMemory(_allocator, _buffer_allocation, &data);
    RB_ASSERT(result == VK_SUCCESS, "Failed to map uniform buffer memory");

    memcpy(data, &material_data, sizeof(material_data));

    vmaUnmapMemory(_allocator, _buffer_allocation);
}

material_vulkan::~material_vulkan() {
    vmaDestroyBuffer(_allocator, _buffer, _buffer_allocation);
}

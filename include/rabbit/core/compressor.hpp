...
namespace rb {
    ...
    class compressor {
    public:
        std::size_t bound(std::size_t uncompressed_size) const;
		...
		template<typename T>
		std::vector<std::uint8_t> compress(const span<const T>& uncompressed_data) const {
		...
		std::vector<T> uncompress(std::size_t uncompressed_size, span<const std::uint8_t> compressed_data) const {
